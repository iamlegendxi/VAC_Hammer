...
const COMMANDS = {
    ...
    "makeDE": async (message, args) => {
        ...
        let desiredNickname = (args[1] ? args.slice(1).join(' ') : (targetMember.nickname === "" ? targetMember.user.username : targetMember.nickname.split('|')[1].trim()));
    ...
    "cut": async (message, args) => {
        ...
        if (!(targetMember.roles.cache.some(r => r.id === franchises[abbrev].role_id))) {
